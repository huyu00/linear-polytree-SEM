...
data_file_name = 'earthquake_data_trial_n500.npz'
# data_file_name = 'earthquake_data_trial_n5000.npz'
...
if flag_load_data:
    ...
    for t in range(ntrial):
        ...
        # hc
        de_hc, ue_hc, runtime_R = hc_R(X1, tag=tag_hc)
    ...
    with open('./data/earthquake_data_trial_n'+str(n)+'.npz','wb') as file1:
...
def print_case_distribution(method_id, method_names, diff_trial):
    ...
    plt.figure()
    ep = 0.2
    plt.scatter(diff_trial[method_id,0,:]+diff_trial[method_id,1,:]+ep*uniform(-1,1,ntrial),
        diff_trial[method_id,2,:]+ep*uniform(-1,1,ntrial), 1*ones(ntrial))
    plt.xlabel('missing+extra')
    plt.ylabel('wrong direction')
    plt.savefig('./figure/earthquake_data_diff_m'+str(method_id)+'.png', dpi=300)
    case_table = zeros((n_edge_true*2+2,n_edge_true+1)) # missing+extra, wrong direction
    for t in range(ntrial):
        x = int(diff_trial[method_id,0,t] + diff_trial[method_id,1,t]) # row=missing+extra
